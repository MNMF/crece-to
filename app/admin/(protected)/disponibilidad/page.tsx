...
export default async function DisponibilidadPage() {
  ...
  // Profesional solo ve/edita su propia disponibilidad
  ...
  // Para el formulario de agregar: admin elige profesional, profesional ya tiene el suyo fijo
  ...
  return (
    <div>
      ...
      <p className="text-ink/60 text-sm mb-6">
        Cada bloque define cuándo aparecen horas disponibles para agendar.
      </p>
      ...
      <div className="space-y-2 mb-10">
        {bloques?.map((b) => (
          <div key={b.id} className="bg-sand rounded-organic p-4 flex items-center justify-between gap-4">
            <div>
              {perfil?.rol === "admin" && b.profesionales && (
                <p className="text-xs text-amber font-medium mb-0.5">
                  {(b.profesionales as { nombre: string }).nombre}
                </p>
              )}
              <span className={!b.activo ? "text-ink/40 line-through" : ""}>
                {dias[b.dia_semana]} · {b.hora_inicio.slice(0, 5)} – {b.hora_fin.slice(0, 5)}
              </span>
            </div>
            <div className="flex gap-2">
              <form action={toggleDisponibilidad.bind(null, b.id, b.activo)}>
              ...
              <form action={eliminarDisponibilidad.bind(null, b.id)}>
                <button className="text-xs text-amber-dark hover:underline px-2">
                  Eliminar
                </button>
              </form>
            </div>
        ...
        {(!bloques || bloques.length === 0) && (
          <p className="text-ink/40 text-sm">Sin bloques configurados aún.</p>
        )}
      </div>
      ...
      <form action={agregarDisponibilidad} className="flex flex-wrap gap-3 items-end">
        {perfil?.rol === "admin" && profesionales && (
          <select
            name="profesional_id"
            required
        ...
        {perfil?.rol === "profesional" && (
          <input type="hidden" name="profesional_id" value={perfil.profesionalId} />
        )}
        <select
          name="dia_semana"
          required
