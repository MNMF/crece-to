...
export default function FormularioAgenda({ profesionales }: { profesionales: Profesional[] }) {
  ...
  useEffect(() => {
    if (!fecha||!profesionalId) { setHorasDisponibles([]); return; }
    setCargandoHoras(true); setHora("");
    fetch(`/api/disponibilidad?fecha=${fecha}&profesionalId=${profesionalId}`)
      .then(r=>r.json()).then(d=>setHorasDisponibles(d.disponibles??[])).finally(()=>setCargandoHoras(false));
  }, [fecha, profesionalId]);

  function handleRutChange(e: React.ChangeEvent<HTMLInputElement>) {
    const raw = limpiarRut(e.target.value);
    if (raw.length<=9) { setRut(raw.length>1?formatearRut(raw):raw); setRutValido(raw.length>=7?validarRut(raw):null); }
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!validarRut(rut)) { setMensajeError("El RUT ingresado no es válido."); setEstado("error"); return; }
    setEstado("enviando"); setMensajeError("");
    const res = await fetch("/api/agendar",{ method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ fecha, hora, servicio_slug:servicioSlug, profesional_id:profesionalId, nombre_paciente:nombre, rut_paciente:rut, telefono, email, notas }) });
    if (res.ok) { setEstado("exito"); }
    else { const d=await res.json(); setMensajeError(d.error||"No se pudo agendar. Intenta de nuevo."); setEstado("error"); }
  }
  ...
  if (estado==="exito") return (
    <div className="bg-sand rounded-organic p-8 text-center">
      ...
      <p className="text-ink/80">Sesión con <strong>{prof?.nombre}</strong> para el <strong>{fecha}</strong> a las <strong>{hora}</strong>. Te contactaremos al {telefono} para confirmar.</p>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <select value={profesionalId} onChange={e=>{setProfesionalId(e.target.value);setFecha("");}} className={inputClass}>
          {profesionales.map(p=><option key={p.id} value={p.id}>{p.nombre} — {p.especialidad}</option>)}
        </select>
      </div>
      <div>
        <label className="block text-sm font-medium mb-1">Servicio</label>
        <select value={servicioSlug} onChange={e=>setServicioSlug(e.target.value)} className={inputClass}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" min={minFecha} required value={fecha} onChange={e=>setFecha(e.target.value)} className={inputClass}/>
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Hora</label>
          <select required disabled={!fecha||cargandoHoras} value={hora} onChange={e=>setHora(e.target.value)} className={`${inputClass} disabled:opacity-50`}>
            <option value="">{cargandoHoras?"Cargando...":!fecha?"Elige una fecha":horasDisponibles.length===0?"Sin horas disponibles":"Selecciona"}</option>
      ...
      <div>
        ...
        <input required value={rut} onChange={handleRutChange} placeholder="12.345.678-9"
          className={`w-full rounded-lg border px-4 py-2.5 bg-white transition-colors ${rutValido===null?"border-sage/40":rutValido?"border-sage":"border-red-400"}`}/>
      ...
      <button type="submit" disabled={estado==="enviando"||!hora||rutValido!==true}
